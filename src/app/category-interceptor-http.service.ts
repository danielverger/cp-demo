...
export const fakeCategoryBackendHttpInterceptor = (categoriesList: Category[]) : HttpInterceptorFn => {
  ...
  return (req: HttpRequest<unknown>, next: HttpHandlerFn) => {
    ...
    if ( url.startsWith( 'categories/' ) && method === 'GET' ) {
      ...
      const user = categoriesList.find( (category: Category) => category.id === categoryId );
      const body = user;
      return of( new HttpResponse({ status: 200, body }) ).pipe(delay(1000));
    }
    ...
    if ( url.startsWith( 'categories' ) && method === 'DELETE' ) {
      const categoryId = parseInt( urlWithParams.split('/')[1] );
      categoriesList = categoriesList.filter( ( category: Category ) => category.id !== categoryId );
      return of( new HttpResponse({ status: 200, body: true }) ).pipe(delay(200));
    }

    if ( url.startsWith( 'categories' ) && method === 'POST' ) {
      const body = { ...(req.body as Category) };
      body.id = Math.max( ...categoriesList.map( ( category:Category ) => category.id ), 0 ) + 1;
    ...
    if ( url.startsWith( 'categories' ) && method === 'PUT' ) {
      ...
      categoriesList = categoriesList.filter( ( category: Category ) => category.id !== idHero );
      categoriesList.push(body as Category);
      return of( new HttpResponse({ status: 200, body }) ).pipe(delay(200));
    }
