...
export class CategoryEditComponent implements OnInit{
  ...
  saveCategory() {
    ...
    const save$ = isNewCategory ? this.categoryService.addCategory(category) : this.categoryService.modifyCategory(category);

    save$.subscribe({
      next: (category:Category) => {
        this.messageService.add({ severity: 'success', summary: 'Successful', detail: `Categoria ${isNewCategory ? 'añadida' : 'modificada'}`, life: 3000 });
        this.ref.close(true)
      },
      error: ( err ) => {
        // this.loading = false;
